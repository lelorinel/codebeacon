...
pub fn dispatch(ctx: &ToolContext, name: &str, args: &Value) -> Result<Value> {
    match name {
        ...
        "call_graph"       => intelligence_handlers::handle_call_graph(ctx, args),
        "review_bundle"    => intelligence_handlers::handle_review_bundle(ctx, args),
        "arch_check"       => intelligence_handlers::handle_arch_check(ctx, args),
        "navigate_to_feature" => intelligence_handlers::handle_navigate_to_feature(ctx, args),
        "test_gaps"        => intelligence_handlers::handle_test_gaps(ctx, args),
        "predict_risk"     => intelligence_handlers::handle_predict_risk(ctx, args),
        "dep_freshness"    => intelligence_handlers::handle_dep_freshness(ctx, args),
        "semantic_search"  => intelligence_handlers::handle_semantic_search(ctx, args),
